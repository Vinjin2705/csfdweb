...
function UniformExemptionPage() {
  ...
  const [requests, setRequests] = useState([
    ...
    },
    {
      id: 2,
      requestNo: '2024-002',
      requestType: 'Religious Exemption',
    ...
    if (selectedRequest) {
      setRequests(prev => prev.map(req => 
        req.id === selectedRequest.id 
          ? { ...req, status: 'approved', dateApproved: new Date().toLocaleDateString() }
          : req
      ))
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <nav className="px-8 py-4 flex items-center justify-between sticky top-0 z-30" style={{ backgroundColor: '#111c4e' }}>
        <div className="flex items-center gap-4">
          ...
          <div className="text-white">
            ...
            <p className="text-xl font-bold">UNIFORM EXEMPTION</p>
          </div>
        </div>
      ...
      <div className="px-8 py-6 max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-b-lg rounded-tr-lg shadow-lg p-6">
          <p className="text-xs text-gray-500 mb-1">{new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}, {new Date().toLocaleDateString('en-US', { weekday: 'long' })}</p>
          <h1 className="text-2xl font-bold mb-4">LIST OF UNIFORM EXEMPTION REQUEST</h1>
          ...
          <div className="space-y-4">
            {filteredRequests.map((request) => (
              <div key={request.id} className="border-2 border-gray-300 rounded-lg p-4 shadow-sm">
                ...
                <div className="mb-3">
                  <p className="text-sm font-medium mb-1">&lt;&lt;{request.requestType}&gt;&gt;</p>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
          <div className="bg-gray-300 rounded-lg w-full max-w-2xl mx-4 p-8">
            <h2 className="text-xl font-bold text-white mb-6" style={{ backgroundColor: '#4a4a4a' }}>
              <span className="inline-block px-4 py-2">Uniform Exemption Request</span>
            </h2>
      ...
      <section className="fixed bottom-8 right-8 flex flex-col items-end gap-3 z-20">
        <div className={`flex flex-col items-end gap-3 transition-all duration-300 ease-in-out ${isFabMenuOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
          ...
          <button onClick={() => navigate('/encode-violation')} className="flex items-center gap-3 px-6 py-3 rounded-full bg-gray-300 text-gray-700 font-medium text-sm hover:bg-gray-400 transition-colors shadow-lg">
