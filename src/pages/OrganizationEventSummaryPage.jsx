...
function OrganizationEventSummaryPage() {
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <section className="px-12 py-12">
        <div className="max-w-4xl mx-auto">
          ...
          {/* Requester Information Summary Box */}
          <div className="border-4 rounded-2xl p-6 mb-6" style={{borderColor: '#111c4e', backgroundColor: '#f9f9f9'}}>
            <h3 className="text-xl font-bold text-center mb-4" style={{color: '#3d3d3d'}}>
              REQUESTER INFORMATION <span style={{color: '#ffc400'}}>SUMMARY</span>
            </h3>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-start">
                ...
                <div>
                  ...
                  <span style={{color: '#111c4e'}}>{requesterData.sex || 'Not provided'}</span>
                </div>
              </div>

              {/* COLLEGE/INSTITUTE/OFFICE/CENTER/ORGANIZATION */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>COLLEGE/INSTITUTE/OFFICE/CENTER/ORGANIZATION: </span>
                <span style={{color: '#111c4e'}}>{requesterData.organization || 'Not provided'}</span>
              </div>

              {/* EMAIL ADDRESS */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>EMAIL ADDRESS: </span>
                <span style={{color: '#111c4e'}}>{requesterData.email || 'Not provided'}</span>
              </div>

              {/* JOB TITLE */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>JOB TITLE: </span>
                <span style={{color: '#111c4e'}}>{requesterData.jobTitle || 'Not provided'}</span>
              </div>

              {/* REQUIREMENTS */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>REQUIREMENT/S: </span>
                <p style={{color: '#111c4e'}}>{requesterData.idFile ? `✓ ${requesterData.idFile.name}` : '<<requirements>>'}</p>
              </div>
            </div>
          </div>

          {/* Event Information Summary Box */}
          <div className="border-4 rounded-2xl p-6 mb-8" style={{borderColor: '#111c4e', backgroundColor: '#f9f9f9'}}>
            <h3 className="text-xl font-bold text-center mb-4" style={{color: '#3d3d3d'}}>
              EVENT INFORMATION <span style={{color: '#ffc400'}}>SUMMARY</span>
            </h3>
            
            <div className="space-y-3">
              {/* EVENT'S NAME */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>EVENT'S NAME: </span>
                <span style={{color: '#111c4e'}}>{eventData.eventTitle || '<<event_name>>'}</span>
              </div>

              {/* EVENT'S VENUE */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>EVENT'S VENUE: </span>
                <span style={{color: '#111c4e'}}>{eventData.venue || '<<event_venue>>'}</span>
              </div>

              {/* EVENT'S DURATION */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>EVENT'S DURATION: </span>
                <span style={{color: '#111c4e'}}>{eventData.eventDate || '<<event_duration>>'}</span>
              </div>

              {/* EXPECTED PARTICIPANT and NO. OF EXPECTED PARTICIPANT */}
              <div className="flex justify-between items-start">
                <div>
                  <span className="font-bold" style={{color: '#111c4e'}}>EXPECTED PARTICIPANT: </span>
                  <p style={{color: '#111c4e'}}>{eventData.expectedParticipantDesc || '<<event_expected_participant>>'}</p>
                </div>
                <div>
                  <span className="font-bold" style={{color: '#111c4e'}}>NO. OF EXPECTED PARTICIPANT: </span>
                  <span style={{color: '#111c4e'}}>{eventData.expectedParticipants || '<<event_expected_participant>>'}</span>
                </div>
              </div>
              ...
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>COLLEGE/INSTITUTE/OFFICE/CENTER/ORGANIZATION: </span>
                <span style={{color: '#111c4e'}}>{eventData.organization || '<<college_institute>>'}</span>
              </div>

              {/* EMAIL ADDRESS OF REQUESTING OFFICE */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>EMAIL ADDRESS OF REQUESTING OFFICE: </span>
                <p style={{color: '#111c4e'}}>{eventData.email || '<<email_address>>'}</p>
              </div>

              {/* CONTACT NUMBER */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>CONTACT NUMBER: </span>
                <span style={{color: '#111c4e'}}>{eventData.contactNumber || '<<contact_number>>'}</span>
              </div>

              {/* REQUIREMENTS */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>REQUIREMENT/S: </span>
                <p style={{color: '#111c4e'}}>{eventData.projectProfileFile ? `✓ ${eventData.projectProfileFile.name}` : '<<requirements>>'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-4">
            <button
              ...
              onClick={() => navigate('/organization-event-info', { state: { requesterData, eventData } })}
            >
              BACK
            </button>
            <button
...
export default OrganizationEventSummaryPage
