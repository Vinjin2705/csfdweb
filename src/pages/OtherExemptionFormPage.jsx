...
function OtherExemptionFormPage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.natureOfRequest.trim()) newErrors.natureOfRequest = 'Nature of request is required'
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <section className="px-12 py-12">
        <div className="max-w-6xl mx-auto">
          ...
          {/* Row: EMAIL ADDRESS, NATURE OF REQUEST */}
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 mb-4">
            ...
            <div>
              <label className="block font-bold text-xs mb-1" style={{color: '#111c4e'}}>
                NATURE OF REQUEST<span style={{color: '#dc2626'}}>*</span>
              </label>
              <input
                type="text"
                className="w-full px-3 py-2 border-2 rounded-lg focus:outline-none text-sm"
                style={{borderColor: errors.natureOfRequest ? '#dc2626' : '#111c4e'}}
                placeholder="Enter the nature of your request"
                value={formData.natureOfRequest}
                onChange={(e) => handleInputChange('natureOfRequest', e.target.value)}
              />
              {errors.natureOfRequest && <p className="text-xs mt-1" style={{color: '#dc2626'}}>{errors.natureOfRequest}</p>}
            </div>
          </div>
          ...
          <div className="space-y-4 mb-8">
            ...
            {/* Requirement 3 - Supporting Document */}
            <div>
              <label className="block font-bold text-xs mb-1" style={{color: '#111c4e'}}>
                REQUIREMENT/S<span style={{color: '#dc2626'}}>*</span>
              </label>
              <p className="text-xs text-gray-600 mb-2">Upload a copy of your additional supporting document. Upload it in PDF format.</p>
              <div 
                className="border-2 border-dashed rounded-lg p-6 text-center cursor-pointer hover:bg-gray-50 transition-colors"
                style={{borderColor: '#111c4e'}}
                onClick={() => document.getElementById('supportingDocFile').click()}
              >
                <input
                  type="file"
                  ...
                  className="hidden"
                  accept=".pdf"
                  onChange={(e) => handleFileChange('supportingDocFile', e.target.files[0])}
                />
                <div className="flex flex-col items-center">
                  <svg className="w-12 h-12 text-gray-400 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                  ...
                  <span className="text-sm text-gray-500">
                    {formData.supportingDocFile ? formData.supportingDocFile.name : 'Add/upload a file'}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-center gap-4">
            ...
            <button
              ...
              onClick={() => {
                if (validateForm()) {
                  navigate('/other-exemption-summary', { state: { formData } })
                }
              }}
            >
              PROCEED
...
export default OtherExemptionFormPage
