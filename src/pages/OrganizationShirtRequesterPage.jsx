...
function OrganizationShirtRequesterPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    adviserName: savedData.adviserName || '',
    adviserEmail: savedData.adviserEmail || '',
    adviserIdFile: savedData.adviserIdFile || null
  })
  ...
  const validateForm = () => {
    ...
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.adviserEmail)) {
      newErrors.adviserEmail = 'Please enter a valid email'
    }
      ...
      <section className="px-12 py-12">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 mb-4">
            ...
            <div>
              <label className="block font-bold text-xs mb-1" style={{color: '#111c4e'}}>
                POSITION IN THE COLLEGE/ORGANIZATION<span style={{color: '#dc2626'}}>*</span>
              </label>
              <input
                type="text"
                className="w-full px-3 py-2 border-2 rounded-lg focus:outline-none text-sm"
                style={{borderColor: errors.position ? '#dc2626' : '#111c4e'}}
                placeholder="Enter your job"
                value={formData.position}
                onChange={(e) => handleInputChange('position', e.target.value)}
              />
              {errors.position && <p className="text-xs mt-1" style={{color: '#dc2626'}}>{errors.position}</p>}
            </div>
          </div>

          {/* Row: ADVISER NAME, ADVISER EMAIL */}
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 mb-6">
            <div>
              <label className="block font-bold text-xs mb-1" style={{color: '#111c4e'}}>
                ADVISER NAME <span style={{color: '#dc2626'}}>*</span>
              </label>
              <input
                type="text"
                className="w-full px-3 py-2 border-2 rounded-lg focus:outline-none text-sm"
                style={{borderColor: errors.adviserName ? '#dc2626' : '#111c4e'}}
                placeholder="Enter the name of the college/organization/council's dean/adviser"
                value={formData.adviserName}
                onChange={(e) => handleInputChange('adviserName', e.target.value)}
              />
              {errors.adviserName && <p className="text-xs mt-1" style={{color: '#dc2626'}}>{errors.adviserName}</p>}
            </div>
            ...
            <div>
              ...
              <input
                type="email"
                className="w-full px-3 py-2 border-2 rounded-lg focus:outline-none text-sm"
                style={{borderColor: errors.adviserEmail ? '#dc2626' : '#111c4e'}}
                placeholder="Enter your email address of organization/council's dean/adviser"
                value={formData.adviserEmail}
                onChange={(e) => handleInputChange('adviserEmail', e.target.value)}
              />
              {errors.adviserEmail && <p className="text-xs mt-1" style={{color: '#dc2626'}}>{errors.adviserEmail}</p>}
            </div>
          </div>
          ...
          <div className="mb-8">
            ...
            <p className="text-xs text-gray-600 mb-2">Upload a copy of official Administrative/Faculty ID of your Organization/Council Adviser.</p>
            <div 
              className="border-2 border-dashed rounded-lg p-8 text-center cursor-pointer hover:bg-gray-50 transition-colors"
              style={{borderColor: '#111c4e'}}
              onClick={() => document.getElementById('adviserIdFile').click()}
            >
              <input
                type="file"
                id="adviserIdFile"
                className="hidden"
                accept=".pdf"
                onChange={(e) => handleFileChange('adviserIdFile', e.target.files[0])}
              />
              <div className="flex flex-col items-center">
                ...
                <span className="text-sm text-gray-500">
                  {formData.adviserIdFile ? formData.adviserIdFile.name : 'Add/upload a file'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-4">
            ...
            <button
              ...
              onClick={() => {
                if (validateForm()) {
                  navigate('/organization-shirt-request', { state: { requesterData: formData } })
                }
              }}
            >
              PROCEED
...
export default OrganizationShirtRequesterPage
