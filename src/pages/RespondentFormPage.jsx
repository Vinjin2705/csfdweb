...
function RespondentFormPage() {
  ...
  const [mainRespondent, setMainRespondent] = useState({
    givenName: savedData.mainRespondent?.givenName || '',
    surname: savedData.mainRespondent?.surname || '',
    middleName: savedData.mainRespondent?.middleName || '',
    extensionName: savedData.mainRespondent?.extensionName || '',
    sex: savedData.mainRespondent?.sex || '',
    studentNumber: savedData.mainRespondent?.studentNumber || '',
    college: savedData.mainRespondent?.college || '',
    email: savedData.mainRespondent?.email || '',
    yearLevel: savedData.mainRespondent?.yearLevel || ''
  })

  const [coRespondents, setCoRespondents] = useState(savedData.coRespondents || [])
  const [coRespondentDropdowns, setCoRespondentDropdowns] = useState([])
  const MAX_CO_RESPONDENTS = 5
  ...
  const addCoRespondent = () => {
    if (coRespondents.length < MAX_CO_RESPONDENTS) {
      ...
      setCoRespondentDropdowns([...coRespondentDropdowns, {
  ...
  const updateCoRespondent = (index, field, value) => {
  ...
  const updateCoDropdown = (index, field, value) => {
    const updated = [...coRespondentDropdowns]
    updated[index] = { ...updated[index], [field]: value }
    setCoRespondentDropdowns(updated)
  }
  ...
  const validateForm = () => {
    ...
    if (!mainRespondent.givenName.trim()) newErrors.main_givenName = 'Given name is required'
    if (!mainRespondent.surname.trim()) newErrors.main_surname = 'Surname is required'
    if (!mainRespondent.sex) newErrors.main_sex = 'Sex is required'
    if (!mainRespondent.studentNumber.trim()) newErrors.main_studentNumber = 'Student number is required'
    if (!mainRespondent.college) newErrors.main_college = 'College is required'
    if (!mainRespondent.email.trim()) {
      newErrors.main_email = 'Email is required'
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(mainRespondent.email)) {
      newErrors.main_email = 'Please enter a valid email'
    }
    if (!mainRespondent.yearLevel) newErrors.main_yearLevel = 'Year level is required'

    // Validate Co-Respondents
    coRespondents.forEach((co, index) => {
  ...
  const renderRespondentSection = (data, setData, prefix, showSexDropdown, setShowSexDropdown, showCollegeDropdown, setShowCollegeDropdown, showYearDropdown, setShowYearDropdown, isCo = false) => (
    <div className="border-2 rounded-xl p-6 mb-6" style={{borderColor: '#111c4e', backgroundColor: 'white'}}>
      <h3 className="text-lg font-bold mb-4" style={{color: '#3d3d3d'}}>
        {isCo ? `CO-RESPONDENT` : 'MAIN RESPONDENT'}
      </h3>
      ...
      <section className="px-12 py-12">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-black" style={{color: '#3d3d3d', fontFamily: 'Metropolis, sans-serif', fontWeight: '900'}}>RESPONDENTS</h1>
            <h2 className="text-2xl font-black" style={{color: '#ffc400', fontFamily: 'Metropolis, sans-serif', fontWeight: '900'}}>INFORMATION</h2>
          </div>

          {/* Main Respondent Section */}
          {renderRespondentSection(
            mainRespondent,
            handleMainChange,
            'main',
            showSexDropdownMain,
          ...
          {/* Co-Respondent Sections */}
          {coRespondents.map((co, index) => (
            <div key={index}>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-bold" style={{color: '#111c4e'}}>CO-RESPONDENT {index + 1}</h3>
                <button
                  className="text-red-600 hover:text-red-800 text-sm font-medium"
                  onClick={() => {
                    const updated = coRespondents.filter((_, i) => i !== index)
                    setCoRespondents(updated)
                    const updatedDropdowns = coRespondentDropdowns.filter((_, i) => i !== index)
                    setCoRespondentDropdowns(updatedDropdowns)
                  }}
                >
                  Remove
                </button>
              ...
              {renderRespondentSection(
                co,
                (field, value) => updateCoRespondent(index, field, value),
                `co${index}`,
                coRespondentDropdowns[index]?.showSexDropdown || false,
                (val) => updateCoDropdown(index, 'showSexDropdown', val),
                coRespondentDropdowns[index]?.showCollegeDropdown || false,
                (val) => updateCoDropdown(index, 'showCollegeDropdown', val),
                coRespondentDropdowns[index]?.showYearDropdown || false,
          ...
          <div className="flex justify-center mb-8">
            {coRespondents.length < MAX_CO_RESPONDENTS ? (
              <button
              ...
              >
                ...
                Add respondents ({coRespondents.length}/{MAX_CO_RESPONDENTS})
              </button>
            ) : (
              <p className="text-sm text-gray-500">Maximum of {MAX_CO_RESPONDENTS} co-respondents reached</p>
            )}
          </div>
          ...
          <div className="flex justify-center gap-4">
            <button
              ...
              onClick={() => navigate('/complaint-form', { state: { formData: { mainRespondent, coRespondents, ...savedData } } })}
            >
              BACK
            </button>
            <button
            ...
            <button
              ...
              onClick={() => {
                if (validateForm()) {
                  navigate('/complaint-details', { state: { formData: { ...savedData, mainRespondent, coRespondents, coRespondentCount: coRespondents.length } } })
                }
              }}
            >
              PROCEED
...
export default RespondentFormPage
