...
function OrganizationShirtSummaryPage() {
      ...
      <section className="px-12 py-12">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="border-4 rounded-2xl p-6 mb-6" style={{borderColor: '#111c4e', backgroundColor: '#f9f9f9'}}>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p style={{color: '#111c4e'}}>{requesterData.organization || '<<college_institute>>'}</p>
              </div>

              {/* POSITION IN THE COLLEGE/ORGANIZATION */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>POSITION IN THE COLLEGE/ORGANIZATION: </span>
                <span style={{color: '#111c4e'}}>{requesterData.position || '<<position>>'}</span>
              </div>

              {/* EMAIL ADDRESS */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>EMAIL ADDRESS: </span>
                <span style={{color: '#111c4e'}}>{requesterData.email || '<<email_address>>'}</span>
              </div>

              {/* ADVISER NAME */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>ADVISER NAME: </span>
                <span style={{color: '#111c4e'}}>{requesterData.adviserName || '<<adviser_name>>'}</span>
              </div>

              {/* ADVISER EMAIL ADDRESS */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>ADVISER EMAIL ADDRESS: </span>
                <span style={{color: '#111c4e'}}>{requesterData.adviserEmail || '<<adviser_email_address>>'}</span>
              </div>

              {/* REQUIREMENTS */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>REQUIREMENT/S: </span>
                <p style={{color: '#111c4e'}}>{requesterData.adviserIdFile ? `✓ ${requesterData.adviserIdFile.name}` : '<<requirements>>'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="border-4 rounded-2xl p-6 mb-8" style={{borderColor: '#111c4e', backgroundColor: '#f9f9f9'}}>
            <h3 className="text-xl font-bold text-center mb-4" style={{color: '#3d3d3d'}}>
              REQUEST INFORMATION <span style={{color: '#ffc400'}}>SUMMARY</span>
            </h3>
            
            <div className="space-y-3">
              {/* EXPECTED DATE OF EFFECTIVITY */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>EXPECTED DATE OF EFFECTIVITY: </span>
                <span style={{color: '#111c4e'}}>{requestData.expectedDate || '<<event_name>>'}</span>
              </div>

              {/* DESIGNATED DAY/S */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>DESIGNATED DAY/S: </span>
                <span style={{color: '#111c4e'}}>{requestData.designatedDays || '<<event_venue>>'}</span>
              </div>

              {/* COLLEGE/INSTITUTE/OFFICE/CENTER/ORGANIZATION */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>COLLEGE/INSTITUTE/OFFICE/CENTER/ORGANIZATION: </span>
                <p style={{color: '#111c4e'}}>{requestData.organization || '<<college_institute>>'}</p>
              </div>

              {/* REQUIREMENTS */}
              <div>
                <span className="font-bold" style={{color: '#111c4e'}}>REQUIREMENT/S: </span>
                <p style={{color: '#111c4e'}}>{requestData.shirtDesignFile ? `✓ ${requestData.shirtDesignFile.name}` : '<<requirements>>'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-4">
            <button
              ...
              onClick={() => navigate('/organization-shirt-request', { state: { requesterData, requestData } })}
            >
              BACK
            </button>
            <button
...
export default OrganizationShirtSummaryPage
